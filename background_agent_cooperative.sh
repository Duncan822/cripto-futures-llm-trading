...
LLM_STRATEGY_GENERATORS=("cogito:8b" "mistral:7b-instruct-q4_0" "phi3:mini" "llama2:7b-chat-q4_0")
LLM_VALIDATORS=("cogito:8b" "mistral:7b-instruct-q4_0")
LLM_OPTIMIZERS=("cogito:8b" "llama2:7b-chat-q4_0")
...
show_help() {
    ...
    echo -e "  ${GREEN}monitor${NC}        - Avvia monitor cooperativo"
    echo -e "  ${GREEN}monitor-stop${NC}   - Ferma monitor cooperativo"
    echo -e "  ${GREEN}monitor-status${NC} - Stato monitor cooperativo"
    echo -e "  ${GREEN}conversations${NC}  - Visualizza conversazioni LLM"
    echo ""
...
get_hardware_optimized_models() {
    ...
    if (( free_mem >= 8 && $(echo "$cpu_usage < 40" | bc -l 2>/dev/null || echo "0") )); then
        ...
        optimized_models=("phi3:mini" "mistral:7b-instruct-q4_0" "cogito:8b")
    else
        # Sistema limitato: usa solo modelli veloci
        optimized_models=("phi3:mini" "mistral:7b-instruct-q4_0")
    fi
...
# ===== FUNZIONI DI MONITORAGGIO COOPERATIVO =====

start_cooperative_monitor() {
    echo -e "${BLUE}🔍 Avvio Monitor Cooperativo...${NC}"
    
    # Controlla se il monitor è già in esecuzione
    if pgrep -f "cooperative_monitor.py" > /dev/null; then
        echo -e "${YELLOW}⚠️ Monitor cooperativo già in esecuzione${NC}"
        return 1
    fi
    
    # Avvia il monitor cooperativo
    nohup python3 cooperative_monitor.py > logs/cooperative_monitor_$(date +%Y%m%d_%H%M%S).log 2>&1 &
    
    # Salva il PID
    echo $! > cooperative_monitor.pid
    
    sleep 3
    if pgrep -f "cooperative_monitor.py" > /dev/null; then
        echo -e "${GREEN}✅ Monitor cooperativo avviato con successo${NC}"
        echo -e "${BLUE}🌐 Dashboard: http://localhost:8081${NC}"
        echo -e "${BLUE}🔗 Monitor LLM base: http://localhost:8080${NC}"
        echo -e "${BLUE}📋 Log: logs/cooperative_monitor_*.log${NC}"
    else
        echo -e "${RED}❌ Errore nell'avvio del monitor cooperativo${NC}"
        return 1
    fi
}

stop_cooperative_monitor() {
    echo -e "${YELLOW}🛑 Arresto Monitor Cooperativo...${NC}"
    
    if [ -f "cooperative_monitor.pid" ]; then
        PID=$(cat "cooperative_monitor.pid")
        if ps -p "$PID" > /dev/null 2>&1; then
            kill "$PID"
            ...
            for i in {1..10}; do
                if ! ps -p "$PID" > /dev/null 2>&1; then
                    echo -e "${GREEN}✅ Monitor cooperativo arrestato${NC}"
                    rm -f "cooperative_monitor.pid"
                    return 0
                fi
                sleep 1
            done
            ...
            rm -f "cooperative_monitor.pid"
            echo -e "${GREEN}✅ Monitor cooperativo arrestato forzatamente${NC}"
        else
            echo -e "${YELLOW}⚠️ Monitor cooperativo non era in esecuzione${NC}"
            rm -f "cooperative_monitor.pid"
        fi
    else
        # Prova a fermare tutti i processi cooperative_monitor
        pkill -f "cooperative_monitor.py" 2>/dev/null || true
        echo -e "${GREEN}✅ Monitor cooperativo arrestato${NC}"
    fi
}

show_cooperative_monitor_status() {
    echo -e "${BLUE}📊 Stato Monitor Cooperativo${NC}"
    echo ""
    
    # Controlla se il monitor è in esecuzione
    if pgrep -f "cooperative_monitor.py" > /dev/null; then
        PID=$(pgrep -f "cooperative_monitor.py")
        echo -e "${GREEN}✅ Monitor cooperativo è in esecuzione${NC}"
        echo -e "   PID: $PID"
        echo -e "   Uptime: $(ps -o etime= -p $PID)"
        echo -e "   Memoria: $(ps -o rss= -p $PID | awk '{print $1/1024 " MB"}')"
        echo ""
        echo -e "${BLUE}🌐 Dashboard disponibili:${NC}"
        echo -e "   🤖 Cooperativo: http://localhost:8081"
        echo -e "   📊 LLM Base: http://localhost:8080"
    else
        echo -e "${RED}❌ Monitor cooperativo non è in esecuzione${NC}"
    fi
    
    echo ""
    echo -e "${BLUE}📁 File di log:${NC}"
    if [ -d "logs" ]; then
        COOP_LOG_COUNT=$(ls logs/cooperative_monitor_*.log 2>/dev/null | wc -l)
        echo -e "   📝 Log monitor cooperativo: $COOP_LOG_COUNT"
        
        if [ $COOP_LOG_COUNT -gt 0 ]; then
            LATEST_COOP_LOG=$(ls -t logs/cooperative_monitor_*.log 2>/dev/null | head -1)
            echo -e "   📄 Ultimo log: $(basename "$LATEST_COOP_LOG")"
        fi
    fi
}

show_conversations() {
    echo -e "${BLUE}💬 Visualizzazione Conversazioni LLM${NC}"
    echo ""
    
    # Controlla se il monitor è in esecuzione
    if ! pgrep -f "cooperative_monitor.py" > /dev/null; then
        echo -e "${RED}❌ Monitor cooperativo non è in esecuzione${NC}"
        echo -e "${YELLOW}💡 Avvia prima il monitor: $0 monitor${NC}"
        return 1
    fi
    
    # Ottieni le conversazioni recenti via API
    echo -e "${YELLOW}📡 Recupero conversazioni recenti...${NC}"
    
    CONVERSATIONS_RESPONSE=$(curl -s http://localhost:8081/api/cooperative/conversations 2>/dev/null)
    
    if [ $? -eq 0 ] && [ -n "$CONVERSATIONS_RESPONSE" ]; then
        echo -e "${GREEN}✅ Conversazioni recuperate${NC}"
        echo ""
        
        # Mostra le ultime 5 conversazioni
        echo -e "${BLUE}📋 Ultime 5 Conversazioni:${NC}"
        echo ""
        
        # Usa Python per formattare le conversazioni
        python3 -c "
import json
import sys

try:
    conversations = json.loads('$CONVERSATIONS_RESPONSE')
    for i, conv in enumerate(conversations[:5]):
        print(f'💬 Conversazione {i+1}:')
        print(f'   🤖 Modello: {conv[\"model\"]}')
        print(f'   🎭 Ruolo: {conv[\"role\"]}')
        print(f'   ⏱️  Durata: {conv[\"duration\"]:.1f}s')
        print(f'   📅 Timestamp: {conv[\"timestamp\"]}')
        if conv.get('validation_score'):
            print(f'   ✅ Punteggio Validazione: {conv[\"validation_score\"]}')
        if conv.get('contest_score'):
            print(f'   🏆 Punteggio Contest: {conv[\"contest_score\"]}')
        print(f'   💭 Prompt: {conv[\"prompt\"][:100]}...')
        print(f'   💬 Risposta: {conv[\"response\"][:100]}...')
        print()
except Exception as e:
    print(f'❌ Errore nel parsing conversazioni: {e}')
"
    else
        echo -e "${RED}❌ Errore nel recupero conversazioni${NC}"
        echo -e "${YELLOW}💡 Verifica che il monitor sia in esecuzione su http://localhost:8081${NC}"
    fi
    
    echo -e "${BLUE}💡 Per visualizzazione completa: http://localhost:8081${NC}"
}
...
start_agent() {
    ...
    nohup python3 background_agent_cooperative.py > "logs/background_agent_cooperative_$(date +%Y%m%d_%H%M%S).log" 2>&1 &
        ...
        cat > "background_config_cooperative.json" << 'EOF'
{
  ...
  "models": ["cogito:8b", "mistral:7b-instruct-q4_0", "phi3:mini", "llama2:7b-chat-q4_0"],
  ...
  "model_selection": {
    "generation": ["cogito:8b", "mistral:7b-instruct-q4_0"],
